...
interface RecipeState {
  ...
  generateRecipeContent: (prompt: string, isIngredientsMode: boolean, userProfile: UserProfile | null) => Promise<void>;
...
function convertToMarkdown(recipe: RecipeStructure): string {
  ...
  md += `# Recipe Details\n`;
  if (recipe.preparationTime) md += `- Preparation Time: ${recipe.preparationTime}\n`;
...
export const useRecipeStore = create<RecipeState>()(
  persist(
    (set, get) => ({
      ...
      setMode: (mode) => set({ mode, recipe: "", parsedRecipe: { title: "", content: "" }, structuredRecipe: null, generationError: null, saved: false }),

      generateRecipeContent: async (prompt, isIngredientsMode, userProfile) => {
        set({ isGenerating: true, recipe: "", structuredRecipe: null, generationError: null, saved: false });
        ...
        try {
          ...
          set({ generationError: "Failed to generate recipe. Please try again." });
        } finally {
